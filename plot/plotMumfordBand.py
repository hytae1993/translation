...
import os
import sys
from util.utils import get_image, get_mum_image, get_threshold_mask, get_mum_image_band
...
class plot:
    ...
    def visualize_loss(self, trainResult, valResult):
        plt.clf()
        figure, axarr = plt.subplots(2, 3, figsize=(24,8))

        axarr[0][0].plot(trainResult[0], 'r-', label='train loss')
        axarr[0][0].fill_between(range(len(trainResult[0])), np.array(trainResult[0])-np.array(trainResult[1]), np.array(trainResult[0])+np.array(trainResult[1]),alpha=.1, color='r')
        axarr[0][0].legend(loc='upper left')
        axarr[0][0].set_title('train total loss')

        axarr[0][1].plot(trainResult[5], 'r-', label='mumford loss')
        axarr[0][1].legend(loc='upper left')
        twin = axarr[0][1].twinx()
        twin.plot(trainResult[2], 'b-', label='TV')
        twin.legend(loc='upper right')
        axarr[0][1].set_title('seg loss')

        axarr[0][2].plot(trainResult[3], 'r-', label='foreground loss')
        axarr[0][2].legend(loc='upper left')
        twin = axarr[0][2].twinx()
        twin.plot(trainResult[4], 'b-', label='background loss')
        twin.legend(loc='upper right')
        axarr[0][2].set_title('mumford loss')

        axarr[1][0].plot(valResult[0], 'r-', label='val loss')
        axarr[1][0].fill_between(range(len(valResult[0])), np.array(valResult[0])-np.array(valResult[1]), np.array(valResult[0])+np.array(valResult[1]),alpha=.1, color='r')
        axarr[1][0].legend(loc='upper left')
        axarr[1][0].set_title('val total loss')

        axarr[1][1].plot(valResult[5], 'r-', label='mumford loss')
        axarr[1][1].legend(loc='upper left')
        twin = axarr[1][1].twinx()
        twin.plot(valResult[2], 'b-', label='TV')
        twin.legend(loc='upper right')
        axarr[1][1].set_title('seg loss')

        axarr[1][2].plot(valResult[3], 'r-', label='foreground loss')
        axarr[1][2].legend(loc='upper left')
        twin = axarr[1][2].twinx()
        twin.plot(valResult[4], 'b-', label='background loss')
        twin.legend(loc='upper right')
        axarr[1][2].set_title('mumford loss')
    ...
    def plotResult(self, epoch, trainResult, valResult):
        path = os.path.join('../../../result/doctor/bi-partitioning/cat/MumfordTest/binarization/', self.config.title + '_iter_' + str(self.config.iter) + \
                    '_ms_' + str(self.config.ms))
