...
class plot:
    ...
    def visualize_stn(self, loader):
        with torch.no_grad():
            ...
            mask, fore, back, foreBack, backFore, foreMask, backMask = get_mum_image(data, self.encoder, self.maskDecoder, self.config.iter)
            ...
            fore_center_grid = self.convert_image_np(
                torchvision.utils.make_grid(fore, nrow=4), False)
            back_center_grid = self.convert_image_np(
                torchvision.utils.make_grid(back, nrow=4), False)

            foreBack_center_grid = self.convert_image_np(
                torchvision.utils.make_grid(foreBack, nrow=4), False)
            backFore_center_grid = self.convert_image_np(
                torchvision.utils.make_grid(backFore, nrow=4), False)
            foreMask_center_grid = self.convert_image_np(
                torchvision.utils.make_grid(foreMask, nrow=4), False)
            backMask_center_grid = self.convert_image_np(
                torchvision.utils.make_grid(backMask, nrow=4), False)

            plt.close('all')
            fig = plt.figure(figsize=(30,18))
            fig.tight_layout()
            ax1 = fig.add_subplot(5,3,1)
            ax11 = fig.add_subplot(5,3,2)
            ax12 = fig.add_subplot(5,3,3)

            ax13 = fig.add_subplot(5,4,5)
            ax14 = fig.add_subplot(5,4,6)
            ax15 = fig.add_subplot(5,4,7)
            ax16 = fig.add_subplot(5,4,8)

            ax2 = fig.add_subplot(5,3,7)
            ax3 = fig.add_subplot(5,3,8)
            ax4 = fig.add_subplot(5,3,9)

            ax5 = fig.add_subplot(5,3,10)
            ax6 = fig.add_subplot(5,3,11)
            ax7 = fig.add_subplot(5,3,12)

            ax8 = fig.add_subplot(5,3,13)
            ax9 = fig.add_subplot(5,3,14)
            ax10 = fig.add_subplot(5,3,15)
            ...
            ax11.set_title('fore mumford:laplace output f')
            ax12.imshow(back_center_grid)
            ax12.set_title('back mumford:laplace output b')

            ax13.imshow(foreBack_center_grid)
            ax13.set_title('f*M + (1-M)*b_mean')
            ax14.imshow(backFore_center_grid)
            ax14.set_title('b*(1-M) + M*f_mean')
            ax15.imshow(foreMask_center_grid)
            ax15.set_title('f*M')
            ax16.imshow(backMask_center_grid)
            ax16.set_title('b*(1-M)')
    ...
    def visualize_loss(self, trainResult, valResult):
        ...
        axarr[2].plot(trainResult[7], 'r-', label='train fore mean for smooth')
        axarr[2].plot(valResult[7], 'g-', label='train back mean for smooth')
        axarr[2].plot(trainResult[8], 'b-', label='val fore mean for smooth')
        axarr[2].plot(valResult[8], 'c-', label='val back mean for smooth')
        axarr[1].set_title('smooth valu')
        axarr[1].legend(loc='upper right')
        
        axarr[3].plot(trainResult[2], 'g-', label='train mask smooth reg')
        axarr[3].plot(valResult[2], 'c-', label='val mask smooth reg')
    ...
    def plotResult(self, epoch, trainResult, valResult):
        path = os.path.join('../../../result/mask/mumford_simple', self.config.title + '_ms_' + str(self.config.ms))
