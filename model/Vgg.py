    ...
    def __init__(self, in_planes, planes, stride=1):
        ...
        if stride == 1:
            ...
            self.deconv2 = nn.ConvTranspose2d(
                planes, planes, kernel_size=3, stride=stride, padding=1, output_padding=1, bias=False
            )
        ...
        if stride != 1:
            self.shortcut = nn.Sequential(
                nn.ConvTranspose2d(
                    in_planes, self.expansion*planes, kernel_size=1, stride=stride, output_padding=1, bias=False
                ),
                nn.BatchNorm2d(self.expansion*planes)
            )
...
class maskNet(nn.Module):
    def __init__(self, in_planes, final_upsample_mode = 'nearest', last=1):
...
def mask18(last):
    return maskNet([64, 128, 256, 512], last=last)