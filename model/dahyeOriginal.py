...
class DecoderSegMulti(nn.Module):
    def __init__(self, n_channels, n_classes, n_last, nearest=True):
        super(DecoderSegMulti, self).__init__()
        self.n_channels = n_channels
        self.n_classes = 3
        self.nearest = nearest
        ...
        self.decrease_depth = DoubleConv(128, 64) # 32x32x128 -> 32x32x64
        self.up1 = UpNoSkipLast(64, n_classes, nearest) # 64x64x64 + 64x64x64 -> 64x64x3
        self.adj_out_depth = OutConv(n_classes, n_last) # 64x64x3 -> 64x64x1
...
class DecoderImagePart(nn.Module):
    def __init__(self, n_channels, out_channels, nearest=True):
        super(DecoderImagePart, self).__init__()
        self.n_channels = n_channels
        self.out_channels = out_channels
        self.nearest = nearest

        self.decrease_depth = DoubleConv(128, 64) # 32x32x128 -> 32x32x64
        self.up1 = UpNoSkipLast(64, out_channels, nearest) # 64x64x64 + 64x64x64 -> 64x64x3
...
class DecoderImage(nn.Module):
    def __init__(self, n_channels, out_channels, nearest=True):
        ...
        self.net_fg = DecoderImagePart(n_channels, out_channels, nearest=True)
        self.net_bg = DecoderImagePart(n_channels, out_channels, nearest=True)
